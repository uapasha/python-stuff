...
from persons.forms import (AddPersonForm, RemovePersonForm,
                           UserForm, UserProfileForm, EditPerson)
...
def manage_persons(request):
    ...
    if form_add.is_valid():
        job = Job.objects.get(id = form_data.get('job'))
        new_person = Person(
...
def edit_person(request, persons_short_name):
    person = get_object_or_404(Person, short_name=persons_short_name)

    form_data = request.POST if request.POST else None
    form_edit = EditPerson(form_data, instance = person)

    if form_edit.is_valid():
        form_edit.save()
        return redirect('/persons/')
    context = {'persons_short_name': persons_short_name,
                    'form_edit': form_edit}
    template = loader.get_template('edit_person.html')

...
from pdatabase_setup import Base, Shelter, Puppy
...
#Add Shelters
shelter1 = Shelter(name = "Oakland Animal Services", max_capacity = 16, current_occupancy = 0, address = "1101 29th Ave", city = "Oakland", state = "California", zipCode = "94601", website = "oaklandanimalservices.org")
session.add(shelter1)

shelter2 = Shelter(name = "San Francisco SPCA Mission Adoption Center", max_capacity = 50, current_occupancy = 0, address="250 Florida St", city="San Francisco", state="California", zipCode = "94103", website = "sfspca.org")
session.add(shelter2)

shelter3 = Shelter(name = "Wonder Dog Rescue", address= "2926 16th Street", max_capacity = 33, current_occupancy = 0,  city = "San Francisco", state = "California" , zipCode = "94103", website = "http://wonderdogrescue.org")
session.add(shelter3)

shelter4 = Shelter(name = "Humane Society of Alameda", address = "PO Box 1571", max_capacity = 24, current_occupancy = 0, city = "Alameda" ,state = "California", zipCode = "94501", website = "hsalameda.org")
session.add(shelter4)

shelter5 = Shelter(name = "Palo Alto Humane Society" ,address = "1149 Chestnut St." , max_capacity = 19, current_occupancy = 0, city = "Menlo Park", state = "California" ,zipCode = "94025", website = "paloaltohumane.org")
...
for i,x in enumerate(male_names):
	...
	shelter = session.query(Shelter).filter(new_puppy.shelter_id==Shelter.id)
	puppies +=1
	shelter[0].current_occupancy +=1
	session.add(new_puppy)
	session.add(shelter[0])
	session.commit() 
...
occupancy = session.query(func.sum(Shelter.current_occupancy)).all()
for _res in occupancy:
	print _res

print "succesfully created %i puppies" % puppies